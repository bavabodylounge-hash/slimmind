...
app.get('/api/ai/debug-claude', async (c) => {
  ...
  if (!apiKey) return c.json({ ...info, claude_test: 'skipped_no_key' })
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
      body: JSON.stringify({
        model: 'claude-haiku-4-5-20251001', max_tokens: 100, temperature: 0.5,
        messages: [{ role: 'user', content: '{"story_lead":"테스트","clinical_ctx":"소견"} 형식으로 짧게 답해주세요.' }],
      }),
    })
    const responseText = await res.text()
    info.claude_status = res.status
    info.claude_ok = res.ok
    info.claude_response_preview = responseText.slice(0, 400)
...
// ── POST /api/ai/debug-generate — generate-story 전 과정 추적 ────────
app.post('/api/ai/debug-generate', async (c) => {
  const apiKey = (c.env as any).ANTHROPIC_API_KEY as string | undefined
  const trace: any = { has_api_key: !!apiKey, steps: [] }
  if (!apiKey) return c.json({ ...trace, error: 'NO_API_KEY' })

  const body = await c.req.json().catch(() => ({})) as any
  const name    = body.name    || '테스터'
  const sex     = body.sex     || '여성'
  const subtype = body.subtype || '식후임산부 가스풍선형'

  const inputJson = JSON.stringify({ name, sex, subtype, bodyChange: null, methods: [], trigger: null, trajectory: { pattern: null, points: [] }, background: null, flags: [] }, null, 2)
  const userMsg = `다음 JSON을 바탕으로 서사(story_lead)와 소견 맥락(clinical_ctx)을 생성해주세요.\n헌법 §1~§7을 전부 지키고, {"story_lead":"...","clinical_ctx":"..."} JSON만 출력하세요.\n\n입력:\n${inputJson}`
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
      body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 800, temperature: 0.7, system: AI_WRITER_SYSTEM, messages: [{ role: 'user', content: userMsg }] }),
    })
    const rawText = res.ok ? ((await res.json() as any).content?.[0]?.text || '') : ''
    trace.steps.push({ step: 'claude_call', status: res.status, ok: res.ok, raw_text_length: rawText.length, raw_text_preview: rawText.slice(0, 600) })

    // JSON 추출
    const jsonMatch = rawText.match(/\{[\s\S]*"story_lead"[\s\S]*"clinical_ctx"[\s\S]*\}/)
    let parsed: any = null
    if (jsonMatch) { try { parsed = JSON.parse(jsonMatch[0]) } catch (e: any) { trace.steps.push({ step: 'json_parse_error', error: String(e) }) } }
    trace.steps.push({ step: 'json_extract', found: !!jsonMatch, parsed_keys: parsed ? Object.keys(parsed) : null })

    if (parsed) {
      // 검증 단계별 체크
      const sl = String(parsed.story_lead || '')
      const cc = String(parsed.clinical_ctx || '')
      const validation_detail: any = {
        story_lead_length: sl.length,
        clinical_ctx_length: cc.length,
        length_ok: sl.length >= 200 && sl.length <= 400 && cc.length >= 60 && cc.length <= 180,
      }
      const FORBIDDEN_STORY = ['상담','예약','가격','업소','컨설턴트','효소','수용체','호르몬 경로','AI','데이터','통계','연구','비교','치료','완치','보장','진단','처방','호르몬','인슐린','대사','코르티솔','도파민','세로토닌','수용체','%','kg','주차','게을렀던 게 아닙니다','다른 구조로 작동']
      const FORBIDDEN_CLINICAL = ['호르몬','인슐린','대사','코르티솔','지방','근육','체중','kg','%','수용체','효소','신경','혈당','혈압','콜레스테롤','내장','림프']
      const story_forbidden = FORBIDDEN_STORY.filter(w => sl.includes(w))
      const clinical_forbidden = FORBIDDEN_CLINICAL.filter(w => cc.includes(w))
      const subtypeCheck = subtype.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
      const has_subtype_in_lead = new RegExp(subtypeCheck).test(sl)
      validation_detail.story_forbidden_found = story_forbidden
      validation_detail.clinical_forbidden_found = clinical_forbidden
      validation_detail.has_subtype_in_lead = has_subtype_in_lead
      validation_detail.overall_ok = validation_detail.length_ok && story_forbidden.length === 0 && clinical_forbidden.length === 0 && !has_subtype_in_lead
      trace.steps.push({ step: 'validation', ...validation_detail })
      trace.story_lead_preview = sl.slice(0, 200)
      trace.clinical_ctx_preview = cc.slice(0, 100)
    }
  } catch (err: any) {
    trace.steps.push({ step: 'exception', error: String(err) })
  }
