...
app.get('/api/b2b/recommend/:resultId', requireB2B(), async (c) => {
  ...
  try {
    ...
    const bcRaw = result.bc_primary as string | null
    if (!bcRaw) return c.json({ error: 'BC코드 미설정', treatments: [] })

    // ✅ [FIX v4.8.2] bc_primary가 닉네임(한글)일 경우 BC 코드로 변환
    // bc_primary가 "스트레스성 야식부엉이형" 처럼 닉네임으로 저장된 경우 LIKE 매칭이 안 됨
    const NICKNAME_TO_BC_RECOMMEND: Record<string, string> = {
      '오후만되면 코끼리다리형':'BC-1','엄마체형 하지정체형':'BC-1',
      '지방흡입후 재발형':'BC-2','목짧아지는 거북이형':'BC-2','짊어진어깨형':'BC-2',
      '아빠체형 내장비대형':'BC-3','식후기절 혈당롤러형':'BC-3','식후기절 혈당롤러코스터형':'BC-3',
      '털털한 PCOS형':'BC-3','남산수박배형':'BC-3','남산수박배 기본형':'BC-3',
      '배부터 무너지는 남성 호르몬 저하형':'BC-3','당이 하체로 가는 저장형':'BC-3','등살부터 차오르는 저장형':'BC-3',
      '약물부작용 강제축적형':'BC-4','억제제부작용 배부름마비형':'BC-4','아빠체형 대사저하형':'BC-4',
      '물만마셔도요요형':'BC-4','물만마셔도요요 기본형':'BC-4','습관이 하체에 쌓인 형':'BC-4',
      '습관이 팔뚝에 쌓인 형':'BC-4','습관이 온몸에 쌓인 형':'BC-4',
      '여름에도 시린 얼음장형':'BC-5','셀룰라이트귤껍질형':'BC-5','셀룰라이트귤껍질 기본형':'BC-5',
      '스트레스성 야식부엉이형':'BC-6','밤에 굳는 하체 정체형':'BC-6','어깨에 얹힌 긴장 축적형':'BC-6',
      '출산후 바람빠진 풍선형':'BC-7','식후임산부 가스풍선형':'BC-7','배만 붓는 복부 정체형':'BC-7',
      '복압 빠진 맥주배형':'BC-7','장이 눌러 상체가 굳는 형':'BC-7',
      '운동할수록 말벅지형':'BC-8','골반틀어짐 승마살형':'BC-8','하체골반 기본형':'BC-8',
      '팔다리거미 올챙이배형':'BC-9','목짧아지는 거북이형(구)':'BC-9',
      '안 쓰는 팔뚝 부종형':'BC-10',
      '상체근육형':'BC-11',
      '겨드랑이 부유방형':'BC-12','가슴 아래 접히는 흉부 정체형':'BC-12',
      '호르몬스위치 갱년기형':'BC-13','갱년기변환형':'BC-13',
      '어깨 뒤부터 바뀌는 호르몬 전환형':'BC-13','온몸이 무거운 전신 정체형':'BC-13',
      '스트레스기절 번아웃형':'BC-14','번아웃무기력형':'BC-14',
      '대사증후군 종합형':'BC-15','대사증후군형':'BC-15','장이 막혀 다리가 무거운 형':'BC-15',
      '다리부터 신호 오는 대사 경고형':'BC-15','목 뒤부터 신호 오는 대사 경고형':'BC-15','장에서 시작된 전신 염증형':'BC-15',
      '동시다발 다중악순환형':'BC-16','다중악순환형':'BC-16','전체적으로둔해진형':'BC-16',
      '축이 무너진 전신 불균형형':'BC-16',
    }
    // BC-1~BC-16 패턴이면 그대로, 닉네임이면 변환
    const bcCode = /^BC-([1-9]|1[0-6])$/.test(bcRaw)
      ? bcRaw
      : (NICKNAME_TO_BC_RECOMMEND[bcRaw] || bcRaw)
